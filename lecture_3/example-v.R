...
par(mfrow=c(1,1))
plot(th.rw,xlab=expression(theta[1]),ylab=expression(theta[2]),pch=16,type="s",
     xlim=range(th1),ylim=range(th2))
contour(th1,th2,den,col=2,add=TRUE,drawlabels=FALSE)
...
par(mfrow=c(1,1))
plot(th.in,xlab=expression(theta[1]),ylab=expression(theta[2]),pch=16,xlim=range(th1),
     ylim=range(th2))
contour(th1,th2,den,col=2,add=TRUE,drawlabels=FALSE)
